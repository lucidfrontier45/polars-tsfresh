...
lf.with_columns(bucket=pl.col("ts").dt.truncate("5m")).group_by("bucket").agg(
    pl.len().alias("events")
)

# Window expression using truncated ts as the over() key
lf.with_columns(bucket_total=pl.col("amount").sum().over(pl.col("ts").dt.truncate("1d")))
```
...
pl.col("address").struct.with_fields(pl.field("city").str.to_uppercase())
